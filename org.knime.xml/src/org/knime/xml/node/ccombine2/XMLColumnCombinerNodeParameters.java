...
/**
 * Node parameters for XML Column Combiner.
 *
...
class XMLColumnCombinerNodeParameters implements NodeParameters {
    ...
    @Persist(configKey = XMLColumnCombinerNodeSettings.NEW_COLUMN)
    @Widget(title = "New column name", description = "Name of the new column in the output table")
    String m_newColumn = "XML";

    @Persist(configKey = XMLColumnCombinerNodeSettings.REMOVE_SOURCE_COLUMNS)
    @Widget(title = "Remove source columns", description = "Check when the source columns should not show up in the "
        + "output table. Source columns are the the columns in the include list and the columns used for the "
        + "<b>data bound name</b> and the <b>data bound attributes</b>.")
    boolean m_removeSourceColumns;

    @Persistor(ColumnFilterPersistor.class)
    @Widget(title = "XML columns to merge",
    description = "The XML columns in the include list will be merged by this node")
    @TypedStringFilterWidgetInternal(hideTypeFilter = true)
    @ChoicesProvider(value = XMLColumnsProvider.class)
    ColumnFilter m_filterConfiguration = new ColumnFilter().withIncludeUnknownColumns();

    @Persistor(ElementNameTypePersistor.class)
    ...
    @Persist(configKey = XMLColumnCombinerNodeSettings.ELEMENT_NAME)
    @Layout(ElementNameSection.class)
    @Widget(title = "Custom element name", description = "Name of the created XML element. A <b>custom name</b> will "
        + "be equal for every row whereas <b>data bound name</b> takes its value from an input column.")
    @Effect(predicate = IsCustomElementName.class, type = EffectType.SHOW)
    String m_elementName = "row";

    @Persist(configKey = XMLColumnCombinerNodeSettings.ELEMENT_NAME_COLUMN)
    @Layout(ElementNameSection.class)
    ...
    @ValueReference(value = ElementNameColumnReference.class)
    ...
    @Widget(title = "Data bound attributes",
        description = "Attributes added to the XML element with data bound values can be defined here")
    @ArrayWidget(elementLayout = ElementLayout.HORIZONTAL_SINGLE_LINE,
        addButtonText = "Add data bound attribute", showSortButtons = false,
        elementDefaultValueProvider = DataBoundAttributeSelectionDefaultValueProvider.class)
    @ValueReference(DataBoundAttributesSelectionRef.class)
    ...
    @ArrayWidget(elementLayout = ElementLayout.HORIZONTAL_SINGLE_LINE,
        addButtonText = "Add custom attribute", showSortButtons = false)
    ...
    static final class ColumnFilterPersistor extends LegacyColumnFilterPersistor {
        ...
        ColumnFilterPersistor() {
            super(XMLColumnCombinerNodeSettings.FILTER_CONFIGURATION);
        }
    ...
    static final class ElementNameTypePersistor extends EnumBooleanPersistor<ElementNameType> {
        ...
        protected ElementNameTypePersistor() {
            super(XMLColumnCombinerNodeSettings.USE_DATA_BOUND_ELEMENT_NAME, ElementNameType.class,
                ElementNameType.DATA_BOUND);
        }
    }
    ...
    static final class DataBoundAttributesPersistor implements NodeParametersPersistor<NameColumnPairSettings[]> {
        ...
        public NameColumnPairSettings[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
            String[] names = settings.getStringArray(
                XMLColumnCombinerNodeSettings.DATA_BOUND_ATTRIBUTE_NAMES, new String[0]);
            String[] columns = settings.getStringArray(
                XMLColumnCombinerNodeSettings.DATA_BOUND_ATTRIBUTE_VALUES, new String[0]);
        ...
        public void save(final NameColumnPairSettings[] obj, final NodeSettingsWO settings) {
            ...
            settings.addStringArray(XMLColumnCombinerNodeSettings.DATA_BOUND_ATTRIBUTE_NAMES, names);
            settings.addStringArray(XMLColumnCombinerNodeSettings.DATA_BOUND_ATTRIBUTE_VALUES, columns);
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{XMLColumnCombinerNodeSettings.DATA_BOUND_ATTRIBUTE_NAMES},
                {XMLColumnCombinerNodeSettings.DATA_BOUND_ATTRIBUTE_VALUES}};
        }
    }
    ...
    static final class CustomAttributesPersistor implements NodeParametersPersistor<NameValuePairSettings[]> {
        ...
        public NameValuePairSettings[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
            String[] names = settings.getStringArray(XMLColumnCombinerNodeSettings.ATTRIBUTE_NAMES, new String[0]);
            String[] values = settings.getStringArray(XMLColumnCombinerNodeSettings.ATTRIBUTE_VALUES, new String[0]);
        ...
        public void save(final NameValuePairSettings[] obj, final NodeSettingsWO settings) {
            ...
            settings.addStringArray(XMLColumnCombinerNodeSettings.ATTRIBUTE_NAMES, names);
            settings.addStringArray(XMLColumnCombinerNodeSettings.ATTRIBUTE_VALUES, values);
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{XMLColumnCombinerNodeSettings.ATTRIBUTE_NAMES},
                {XMLColumnCombinerNodeSettings.ATTRIBUTE_VALUES}};
        }
