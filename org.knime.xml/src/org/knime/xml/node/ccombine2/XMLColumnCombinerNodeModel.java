...
public class XMLColumnCombinerNodeModel extends NodeModel {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        if (null == m_settings.getNewColumn()) {
            ...
            m_settings.setNewColumn(
                    DataTableSpec.getUniqueColumnName(spec, "XML"));
        }
        ...
        DataColumnSpecFilterConfiguration filterConfiguration = m_settings.getFilterConfiguration();
        if (filterConfiguration == null) {
            DataColumnSpecFilterConfiguration config = createDCSFilterConfiguration();
            m_settings.setFilterConfiguration(config);
            filterConfiguration = m_settings.getFilterConfiguration();
        }

        FilterResult filter = filterConfiguration.applyTo(spec);
        String[] rmFromIncl = filter.getRemovedFromIncludes();
        if (filterConfiguration.isEnforceInclusion() && rmFromIncl.length != 0) {
        ...
        i++) {
            ...
            if (spec.findColumnIndex(column) == -1) {
    ...
    private ColumnRearranger createColumnRearranger(final DataTableSpec spec)
        ...
        String[] includeColumnNames = m_settings.getFilterConfiguration().applyTo(spec).getIncludes();
